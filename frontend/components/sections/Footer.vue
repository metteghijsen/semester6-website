<template>
  <div class="bg-black10">
    <div class="w-full bg-white100 grid grid-cols-4 gap-2 px-[450px] py-[120px]" data-aos="fade-up" data-aos-delay="50">
      <UiLogo image-name="fontys" to="https://www.fontys.nl/Home.htm" />
      <UiLogo image-name="kea" to="https://kea.dk/en/" />
      <UiLogo image-name="livewall" to="https://www.livewall.nl/" />
      <UiLogo image-name="sogeti" to="https://www.sogeti.nl/" />

      <UiLogo image-name="9292" to="https://9292.nl/" class="p-4" />
      <UiLogo image-name="warnermusic" to="https://www.wmg.com/" />
      <UiLogo image-name="partou" to="https://www.werkenbijpartou.nl/?utm_source=googleads&utm_campaign=18773651246&utm_medium=ad&utm_content=670754134683&utm_term=werken+bij+partou&gclid=Cj0KCQjw06-oBhC6ARIsAGuzdw0MH2NWr98Skszax908DrasgF6UXzMTW2UV8SCRp436CjrR_p6BBcQaAvYkEALw_wcB" />
      <UiLogo image-name="salvemundi" to="https://www.salvemundi.nl" />
    </div>
    <div class="flex flex-row justify-evenly py-[120px]" data-aos="fade-up" data-aos-delay="50">
      <div>
        <UiTypography type="h4" size="heading4" class="pb-5">
          S6 Advanced Media Design <br>
          Research Based Learning
        </UiTypography>
        <UiTypography type="p" size="paragraph">
          Mette Ghijsen © 2023
        </UiTypography>
      </div>
      <div>
        <UiTypography type="h4" size="heading4" class="pb-0.5">
          Say hello
        </UiTypography>
        <a href="mailto:info@metteghijsen.nl" target="_blank">
          <UiTypography type="p" size="paragraph" class="pb-6">
            →  info@metteghijsen.nl
          </UiTypography>
        </a>
        <UiTypography type="h4" size="heading4" class="pb-0.5">
          Social media
        </UiTypography>
        <a href="https://github.com/metteghijsen" target="_blank">
          <UiTypography type="p" size="paragraph">
            →  GitHub
          </UiTypography>
        </a>
        <a href="https://www.linkedin.com/in/mette-ghijsen-857594203/" target="_blank">
          <UiTypography type="p" size="paragraph">
            →  LinkedIn
          </UiTypography>
        </a>
      </div>
    </div>
  </div>
</template>
